...
function normalCdf(x) {
  if (x === 0) return 0.5; // handles both +0 and -0; polynomial is not exact at 0
...
function mcSimulate(S, K, T, sigma, r, N, type) {
...
if (typeof module !== 'undefined') {
  module.exports = { normalCdf, boxMuller, bsPrice, logCheckpoints, mcSimulate, runSimulation };
}

// Entry point called by mcop-worker.js
function runSimulation({ S, K, T, sigma, r, N, type }) {
  const bs = bsPrice(S, K, T, sigma, r, type);
  const mc = mcSimulate(S, K, T, sigma, r, N, type);
  return {
    bs_price: bs.price,
    bs_d1: bs.d1,
    bs_d2: bs.d2,
